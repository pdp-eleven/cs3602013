const char** rx(unsigned int tcpPort, unsigned char IPv4Address[], MemBoundry dataOut){
  ...
  if(bind(rxSocket, (LPSOCKADDR)&rxInfo, sizeof(sockaddr)) == SOCKET_ERROR){
    WSACleanup();
    return (const char**)utk::NET_FAILURE_BIND_SOCKET;
  }

    
  if( listen(rxSocket, 1) == SOCKET_ERROR ){
    WSACleanup();
    return (const char**)utk::NET_FAILURE_LISTEN_SOCKET;
  }
  ...
  if( txSocket == INVALID_SOCKET ){
    WSACleanup();
    return (const char**)utk::NET_FAILURE_ACCEPT_SOCKET;
  }
  ...
  if(dataOut.lowAddress == NULL){
    return (const char**)utk::NET_FAILURE_BUFFER_ALLOCATION;
  }
